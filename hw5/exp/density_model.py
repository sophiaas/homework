...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.kl = tf.add(tf.distributions.kl_divergence(self.encoder1, self.prior), tf.distributions.kl_divergence(self.encoder2, self.prior))
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.get_variable("z_logstd", shape=(z_size,), initializer=tf.zeros_initializer, trainable=True)
    ...
    def make_prior(self, z_size):
        ...
        # prior_logstd = tf.constant(tf.exp(1), shape=z_size)
    ...
    def get_likelihood(self, state1, state2):
        ...
        # encoder1, encoder2, prior, discriminator = self.forward_pass(self, state1, state2)
        target = np.ones((state1.shape[0], 1))
        # likelihood = tf.squeeze(self.discriminator.log_prob(target), axis=1)
